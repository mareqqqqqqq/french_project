...
class LessonService:
    ...
    async def add_vocabulary_card(self, teacher_id: int, lesson_id: int, word_fr: str, word_ru: str, emoji: str = None):
        lesson = await self.repo.get_lesson_by_id(lesson_id)
        if teacher_id != lesson.teacher_id:
            raise NotAuthorizedException(message=f"недостаточно прав")

        vocabulary_card = await self.repo.add_vocabulary_card(lesson_id, word_fr, word_ru, emoji)
        return vocabulary_card

    async def add_fill_blank(self, teacher_id: int, lesson_id: int, sentence: str, translation: str, correct_answer: str, options: list,):
        lesson = await self.repo.get_lesson_by_id(lesson_id)
        if teacher_id != lesson.teacher_id:
            raise NotAuthorizedException(message="Недостаточно прав")

        fill_blank = await self.repo.add_fill_blank(lesson_id, sentence, translation, correct_answer, options)
        return fill_blank